...
const getPaceRecommendations = async (userId, activityId) => {
  ...
  return new Promise((resolve, reject) => {
    onValue(lapsRef, async (snapshot) => {
      ...
      try {
        await savePaceRecommendations(userId, activityId, paceRecommendations);
...
const savePaceRecommendations = async (userId, activityId, paceRecommendations) => {
  const db = getDatabase();
  const paceRef = ref(db, `users/${userId}/activities/${activityId}/paceRecommendations`);
